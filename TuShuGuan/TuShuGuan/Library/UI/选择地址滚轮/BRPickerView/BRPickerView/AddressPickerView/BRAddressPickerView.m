...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        ...
        proviceModel.region_id = proviceDic[@"id"];
        proviceModel.region_name = proviceDic[@"name"];
        ...
        for (NSDictionary *cityDic in citylist) {
            ...
            cityModel.region_id = cityDic[@"id"];
            cityModel.region_name = cityDic[@"name"];
            ...
            for (NSDictionary *areaDic in arealist) {
                ...
                areaModel.region_id = areaDic[@"id"];
                areaModel.region_name = areaDic[@"name"];
...
- (void)setupDefaultValue {
    ...
    if (_defaultSelectedArr) {
        ...
        //TODO WY
        if (_defaultSelectedArr.count > 2 && [_defaultSelectedArr[2] isKindOfClass:[NSString class]] && ![_defaultSelectedArr[2]  isEqual: @""]) {
    ...
    [self.provinceModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([model.region_name isEqualToString:selectProvinceName]) {
            _provinceIndex = idx;
    ...
    if (self.showType == BRAddressPickerModeCity || self.showType == BRAddressPickerModeArea) {
        ...
        [self.cityModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([model.region_name isEqualToString:selectCityName]) {
                _cityIndex = idx;
    ...
    if (self.showType == BRAddressPickerModeArea) {
        ...
        [self.areaModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([model.region_name isEqualToString:selectAreaName]) {
                _areaIndex = idx;
...
- (BRCityModel *)selectCityModel {
    if (!_selectCityModel) {
        ...
        _selectCityModel.region_id = @"";
        _selectCityModel.region_name = @"";
    }
...
- (BRAreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        ...
        _selectAreaModel.region_id = @"";
        _selectAreaModel.region_name = @"";
    }
