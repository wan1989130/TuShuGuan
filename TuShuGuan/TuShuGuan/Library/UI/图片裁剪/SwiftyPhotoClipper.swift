...
class SwiftyPhotoClipper: UIViewController {
    ...
    // 截图大小
    var selectWidth:CGFloat = UIScreen.main.bounds.width < 358 ? UIScreen.main.bounds.width : 358
    var selectHeight:CGFloat = 441
...
//        okBtn.setTitleColor(UIColor.black, for: .normal)
    ...
    fileprivate func clipImage()->UIImage?{
        ...
        guard let img = UIGraphicsGetImageFromCurrentImageContext()?.cgImage,
            ...
            let result = img.cropping(to: CGRect(x: scal * (lineWidth + x), y: (HEIGHT - selectHeight)/2 * scal, width: (selectWidth - 2*lineWidth) * scal, height: selectHeight * scal))   else{
                return nil
        }
        // 关闭上下文
...
//        context?.stroke(CGRect(x: 0, y: (HEIGHT - selectHeight)/2 - lineWidth , width: WIDTH , height: selectHeight + 2*lineWidth))
        context?.stroke(CGRect(x: WIDTH < 358 ? 0 : (WIDTH - 358)/2, y: (HEIGHT - selectHeight)/2, width: selectWidth , height: selectHeight))
        context?.strokePath()
