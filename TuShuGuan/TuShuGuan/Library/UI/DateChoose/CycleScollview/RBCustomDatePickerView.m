...
- (void)setDayScrollViewNew:(NSInteger)dayIntTmp
{
    if (dayScrollView){
    ...
    if (dayIntTmp >= [self DaysfromYear:yearIndex andMonth:monthIndex]){
        dayIntTmp = [self DaysfromYear:yearIndex andMonth:monthIndex];
    }
    NSInteger dayint = dayIntTmp;
    [dayScrollView setCurrentSelectPage:(dayint-3)];
    
    dayScrollView.delegate = self;
    dayScrollView.datasource = self;
    NSLog(@"dayCount = %d,day=%d",dayCount,dayint);
    [self setAfterScrollShowView:dayScrollView andCurrentPage:1];
    [timeBroadcastView addSubview:dayScrollView];
}
//设置年月日时分的滚动视图
...
//    NSLog(@"resultTimeStr - %@",resultTimeStr);
    if (scrollView == yearScrollView || scrollView == monthScrollView){
        dayCount = [self DaysfromYear:yearInt andMonth:monthInt];
        yearIndex = yearInt;
        monthIndex = monthInt;
        [self setDayScrollViewNew:dayInt];
    }
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
